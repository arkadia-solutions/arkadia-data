...
def highlight_output(text: str, fmt: str) -> str:
    ...
    if fmt == "json":
        ...
        # Numbers/Bool/Null
        text = re.sub(
            r":\s*(-?\d+\.?\d*|true|false|null)", f": {C.BLUE}\\1{C.RESET}", text
        )
        return text
...
def run(args):
    ...
    if errors:
        print(
            f"\n{C.RED}Decoding Failed with {len(errors)} Errors:{C.RESET}",
            file=sys.stderr,
        )
    ...
    try:
        ...
        print(
            f"{C.RED}Error converting Node to Python object:{C.RESET} {e}",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    try:
        if fmt == "json":
            output_text = json.dumps(
                py_data,
                indent=args.indent if not args.compact else None,
                ensure_ascii=False,
            )
            if use_color:
    ...
    if args.output:
        try:
            ...
            if not args.only_data:
                print(
                    f"{C.GREEN}Success!{C.RESET} Decoded {fmt.upper()} saved to: {C.BOLD}{out_path}{C.RESET}"
                )
...
def register_arguments(parser):
    ...
    # TOON Specific
    parser.add_argument(
        "--delimiter", default=",", help="[TOON] Array delimiter (default: ,)"
    )
    parser.add_argument(
