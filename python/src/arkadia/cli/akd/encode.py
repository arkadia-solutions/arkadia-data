...
def show_encode_help():
    ...
    io_flags = [
        ...
        {
            "flags": "-c, --compact",
            "desc": "Enable compact mode (minified, no whitespace)",
        },
    ...
    color_flags = [
        ...
        {
            "flags": "--no-color",
            "desc": "Disable syntax highlighting (even in terminal)",
        },
    ]
    ...
    enc_flags = [
        ...
        {
            "flags": "--prompt-output",
            "desc": "Optimize output format for LLM prompting",
        },
    ]
...
def run(args):
    ...
    if args.input and args.input != "-":
        ...
        if not input_path.exists():
            print(
                f"{C.RED}Error: File not found: {input_path}{C.RESET}", file=sys.stderr
            )
            sys.exit(1)
    ...
    # Handle Stdin (explicit '-' or implicit pipe)
        ...
        if not sys.stdin.isatty() or args.input == "-":
            try:
                ...
                data = json.loads(
                    content
                )  # Assuming stdin is JSON; could add format detection if needed
            except Exception as e:
            ...
            print(
                f"{C.RED}Error: Input required (file or stdin).{C.RESET}",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if args.output:
        try:
            ...
            print(
                f"{C.GREEN}Success!{C.RESET} Encoded data saved to: {C.BOLD}{out_path}{C.RESET}"
            )
        except Exception as e:
...
def register_arguments(parser):
    ...
    color_group.add_argument(
        "--colorize", action="store_true", help="Force syntax highlighting"
    )
    color_group.add_argument(
    ...
    parser.add_argument(
        "--include-array-size", action="store_true", help="Add @size annotation"
    )

    # Negative flags (default True in logic)
    parser.add_argument(
        "--no-schema", action="store_true", help="Disable schema header"
    )
    parser.add_argument(
        "--no-type", action="store_true", help="Disable type annotation"
    )
    parser.add_argument(
        "--no-meta", action="store_true", help="Disable metadata blocks"
    )

    parser.add_argument(
        "--prompt-output", action="store_true", help="LLM prompt optimized"
    )
